...
One such example is the accelerometer. Access to accelerometers is usually unrestricted on most of the current mobile phone operating systems, due to the frequent use of accelerometer data to determine and adjust window orientation. Furthermore, the accelerometer is very energy efficient and uses about 10 times less power than other motion sensors \cite{android}. [\textbf{citation}]
% \url{http://developer.android.com/guide/topics/sensors/sensors_motion.html#sensors-motion-accel}

In this paper, we present \textbf{EavesDroid}, an application to recover keystrokes using 
accelerometer data. We demonstrate that \textbf{EavesDroid} is able to record and reconstruct the keypresses made on a nearby keyboard with decent accuracy, based solely on the observed vibrations. We develop profiles for different keys by extracing features from the keypress signals and classify them using boosted Random Forests. This allows us to establish an abstract representation of the relationship between a keystroke signal and its corresponding key. We then recover the typed content
by translating from our intermediary form to English words using a number
of different dictionaries.
While there have been similar effors to reconstruct keystrokes from recorded audio signals with high accuracy [\textbf{cite}],
...
data from the remaining $1/3^{rd}$ letters data for the 72 sentences. Using the work flow
...
EavesDroid is able to correctly recover 5.46\% of the 4490 words. Table~\ref{tab:harvard-labels}
shows the percentage of words which at most the corresponding number of errors in the combined
LR and UD label predictions. Note that 97.27\% of the words have at most 6 mis-predictions using
the EavesDroid's classifiers. 6 mis-predictions does not mean that all the label predictions
on a 6 letter word are incorrect. Instead for a 6 letter word, 12 labels are predicted and out of
...
The module is able to successful recover 121 out of the 138 dictionary words with at most
5 labeling errors. This demonstrates the practical utility of EavesDroid in recovering text
...
\begin{tabular}{|c|c|}
\hline
\textbf{\# errors (L/R, U/D)} & \textbf{Recovered Words Accuracy(\%)}\\
\hline
0 & 5.46\\
\hline
1 & 23.41\\
